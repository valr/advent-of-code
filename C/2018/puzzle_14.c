...
static void how_many_recipes_appear_on_the_scoreboard_to_the_left_of_the_first_recipes_whose_score_are_the_digits_from_the_input(void)
{
    unsigned long long value =
        ...
        list->prev->prev->prev->value * 100ULL +
        list->prev->prev->value * 10ULL +
        list->prev->value;
...
int main (int argc, char **argv)
{
    ...
    recipe *r = list->prev->prev->prev->prev->prev->prev->prev->prev->prev->prev->prev->prev;
    printf("answer 1: ");
    for (int i = 0; i < 10; i++) {
        printf("%d", r->value);
        r = r->next;
